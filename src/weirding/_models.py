...
def build_model(schema: JsonSchemaIR, *, name: str = "Model") -> type[BaseModel]:
    ...
    Patches applied here (not in the library):
      ...
      - schema["description"] (top-level)  →  generated class __doc__ (ADR-0011);
        absent when the schema has no description (stays None)
    """
    try:
    ...
    # Set the class __name__ to the caller-supplied name; propagate the schema's
    # top-level description into __doc__ (ADR-0011). __doc__ is not inherited from
    # the base, so an absent description leaves it None.
    namespace: dict[str, Any] = {"__module__": model.__module__}
